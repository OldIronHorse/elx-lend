defmodule LendBookTest do
  ...
  test "empty book" do
    assert %{borrow: [], lend: []} = Lend.Book.new(60)
  end
  ...
  test "add borrow to empty book" do
    assert %{
             ...
             Lend.Book.add(Lend.Book.new(60), %Lend.Order{side: :borrow, size: 10000, rate: 0.05})
  end
  ...
  test "add lend to empty book" do
    assert %{
             ...
             borrow: []
           } = Lend.Book.add(Lend.Book.new(60), %Lend.Order{side: :lend, size: 10000, rate: 0.05})
  end
  ...
  test "cross: empty book" do
    assert Lend.Book.cross(Lend.Book.new(60)) == {Lend.Book.new(60), []}
  end
  ...
  test "cross: partial borrow" do
    assert {
             ...
             [%Lend.Loan{rate: 0.04, size: 6000, lender: "Rich", borrower: "Pete", term: 60}]
           } ==
             Lend.Book.new(60)
             |> Lend.Book.add(Lend.Order.new(:borrow, 10000, 0.04, "Pete"))
             |> Lend.Book.add(Lend.Order.new(:borrow, 11000, 0.03, "Bill"))
